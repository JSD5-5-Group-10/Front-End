  ...
  return (
    <div className="">
      ...
      <div className="place-items-center grid grid-cols-1 gap-5 lg:grid-cols-3 md:grid-cols-2 md:mx-[10px] md:gap-y-[110px] lg:gap-y-[80px] mx-5">
        {data.map((item, index) => (
          <div className="relative w-[350w] md:w-auto md:h-auto" key={index}>
            <Link to={item.router}>
              <img
                src={item.imgSrc}
                alt={item.altText}
            ...
            <div className="textContainer">
              <h3 className="absolute text-4xl md:text-2xl lg:text-3xl text-white font-medium top-1/3 left-1/3 -translate-x-1/2 -translate-y-1/2">
                {item.title}
              </h3>
